...
| Скрипт | Когда брать |
|---|---|
| `./run.sh` | **Боевой режим без демо-данных.** Поднимает чистый стенд. `.env` создаётся пустой — настроишь через UI «Настройки» или вручную. |
| `./run_demo.sh` | **Demo с данными.** `.env.demo` (mock-LLM, mock-embeddings) + автоматический сидинг 200 синтетических тикетов. |
| `./run_fresh.sh` | **Боевой, начисто.** Сносит `.venv`, БД, `.env`, токен и поднимает с нуля без демо-данных. |
| `./run_demo_fresh.sh` | **Demo с данными, начисто.** Полное обнуление + восстановление демо-стенда. |

Все 4 скрипта при первом запуске интерактивно спросят токен корп-pypi-зеркала (если нужен) и сохранят его в `.sber_pypi_token`. Дальше — `.venv`, зависимости, миграции, uvicorn на http://127.0.0.1:8000/ui.

Дополнительные флаги (работают для всех скриптов):

```bash
./run.sh --reset         # пересоздать только БД, оставить venv и .env
./run.sh --reset-deps    # переустановить зависимости
./run.sh --no-install    # пропустить pip install (быстрый старт)
./run.sh --no-prompt     # без интерактива (для CI)
./run.sh --port 8080
